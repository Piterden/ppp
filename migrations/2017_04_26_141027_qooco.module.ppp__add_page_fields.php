...
class QoocoModulePppAddPageFields extends Migration
{
    ...
    {
        if($default_page_layout=$this->fields()->findBySlugAndNamespace('default_page_layout','ppp')){
            $default_page_layout->delete();
        }
        if($default_post_type=$this->fields()->findBySlugAndNamespace('default_post_type','ppp')){
